  ...
  return (
    <section className="rounded-2xl border border-mist-light bg-paper p-5">
      <header className="flex items-baseline justify-between gap-3 mb-4">
        <div>
          <div className="text-[10px] tracking-[0.18em] text-midnite uppercase font-medium mb-1">Sales-prioriteit</div>
          <h2 className="text-[14px] font-semibold text-ink">Hete leads</h2>
        ...
        <div className="text-[11px] text-ink-mute">{top.length} top</div>
      </header>
      {top.length === 0 || top[0].score === 0 ? (
        <div className="rounded-xl border border-dashed border-mist-light bg-canvas px-4 py-8 text-center text-[13px] text-ink-soft">
        ...
        <ol className="space-y-1.5">
          {top.map(({ session, score }, idx) => (
            ...
            >
              <div className="text-[11px] text-ink-mute tabular-nums text-center">#{idx + 1}</div>
              <div className="min-w-0">
                <div className="text-[13px] font-medium text-ink truncate flex items-center gap-2">
                  ...
                  {session.lead?.email && (
                    <span className="text-[10.5px] text-ink-mute truncate">· {session.lead.email}</span>
                  )}
                </div>
                <div className="text-[10.5px] text-ink-mute mt-0.5 flex items-center gap-1.5 truncate">
...
function ScoreChip({ value }) {
  ...
  return (
    <div className={'rounded-full border px-2 py-0.5 text-[11px] font-semibold tabular-nums text-center ' + cls}>
      {value}
    </div>
  )
}
