  ...
  return (
    <section id="settings" className="rounded-2xl border border-mist-light bg-paper p-5 col-span-full scroll-mt-24">
      <header className="flex items-baseline justify-between gap-3 mb-4">
        <div>
          <div className="text-[11px] tracking-[0.18em] text-midnite uppercase font-medium mb-1">Settings</div>
      ...
      >
        <span
          className={
            'shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-[13px] font-semibold mt-0.5 ' +
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-[12.5px] text-ink-soft mt-0.5 leading-relaxed flex items-center gap-2 flex-wrap">
            <span>Huidig IP:</span>
            <code className="bg-canvas-2 px-1.5 py-0.5 rounded text-[12px] tabular-nums">
            ...
            {currentIp && !myIpExcluded && (
              <button
                type="button"
                onClick={onExcludeMe}
                className="ml-auto text-[12.5px] text-paper bg-midnite hover:bg-midnite-soft px-3 py-1 rounded-full transition whitespace-nowrap"
              >
                Sluit mijn IP uit
              </button>
            )}
            {myIpExcluded && (
              <span className="ml-auto text-[12.5px] text-emerald-700 font-medium">
      ...
      <div className="grid grid-cols-[1fr_auto] gap-2 mb-4">
        <input
          ...
          className="rounded-lg border border-mist focus:border-midnite outline-none text-[14px] px-3 py-2"
        />
        <button
          type="button"
          onClick={onAddManual}
          disabled={!manualIp.trim()}
          className="text-[13px] text-paper bg-midnite hover:bg-midnite-soft disabled:opacity-50 disabled:cursor-not-allowed px-4 py-2 rounded-lg transition"
        >
          Toevoegen
        </button>
      </div>
      ...
      <div>
        <div className="flex items-baseline justify-between mb-2">
          <div className="text-[11px] tracking-[0.18em] text-ink-mute uppercase font-medium">
          ...
          {excluded.length > 0 && (
            <button
              type="button"
              onClick={onClearAll}
              className="text-[12px] text-rose-700 hover:text-rose-900 transition"
            >
              Alle wissen
            </button>
          )}
        ...
        {excluded.length === 0 ? (
          <div className="rounded-xl border border-dashed border-mist-light bg-canvas px-4 py-5 text-center text-[13px] text-ink-mute">
            ...
            {excluded.map((ip) => {
              ...
              return (
                <li key={ip} className="flex items-center justify-between gap-2 px-3.5 py-2.5">
                  <code className="text-[13.5px] text-ink tabular-nums truncate">{ip}</code>
                  <div className="flex items-center gap-2 shrink-0">
                    {isCurrent && (
                      <span className="text-[11px] uppercase tracking-wider text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded">
                        jouw IP
                      </span>
                    )}
                    <button
                      type="button"
                      onClick={() => onRemove(ip)}
                      className="text-[12px] text-rose-700 hover:text-rose-900 hover:bg-rose-50 px-2 py-0.5 rounded transition"
                    >
                      Verwijderen
                    </button>
                  </div>
