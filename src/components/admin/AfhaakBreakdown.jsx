...
export default function AfhaakBreakdown({ byStep, byReason }) {
  ...
  return (
    <section className="rounded-2xl border border-mist-light bg-paper p-5">
      ...
      <p className="text-[13px] text-ink-soft mb-5">Waar haken bezoekers af, en waarom.</p>

      <div className="grid md:grid-cols-2 gap-6">
        <div>
          <div className="text-[11px] tracking-[0.18em] text-ink-mute uppercase mb-3">Bij welke stap</div>
          {totalStep === 0 ? (
            <div className="text-[14px] text-ink-mute py-3">Nog geen afhakers.</div>
          ) : (
            <ul className="space-y-1.5">
              {byStep.slice(0, 8).map((row) => {
                const pct = (row.count / totalStep) * 100
                return (
                  <li key={row.type} className="flex items-baseline justify-between gap-3 text-[13px]">
        ...
        <div>
          <div className="text-[11px] tracking-[0.18em] text-ink-mute uppercase mb-3">Reden bij brochure-nee</div>
          {totalReason === 0 ? (
            <div className="text-[14px] text-ink-mute py-3">Nog geen redenen gegeven.</div>
          ) : (
            <ul className="space-y-1.5">
              {byReason.map((row) => {
