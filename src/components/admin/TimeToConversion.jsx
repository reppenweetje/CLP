  ...
  return (
    <section className="rounded-2xl border border-mist-light bg-paper p-5">
      <header className="flex items-baseline justify-between gap-3 mb-4">
        <div>
          <div className="text-[10px] tracking-[0.18em] text-midnite uppercase font-medium mb-1">Tempo</div>
          <h2 className="text-[14px] font-semibold text-ink">Tijd tot conversie / verlaten</h2>
        </div>
        ...
        {data.completed.map((b, i) => {
          ...
          return (
            <div key={b.id} className="grid grid-cols-[64px_1fr_64px] items-center gap-3">
              <div className="text-[11px] text-ink-mute text-right tabular-nums">{b.label}</div>
              ...
              <div className="text-[10px] text-ink-mute tabular-nums text-right">
      ...
      <div className="mt-4 pt-3 border-t border-mist-light flex items-center justify-between text-[11px] text-ink-mute">
...
function Bar({ value, max, color, labelLeft, labelRight }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="text-[10px] text-ink-mute w-16 shrink-0">{labelLeft}</div>
      <div className="flex-1 h-3 bg-canvas-2 rounded-full overflow-hidden relative">
        <div className={'h-full ' + color + ' transition-all duration-500'} style={{ width: `${pct}%` }} />
      </div>
      <div className="text-[10px] text-ink tabular-nums w-8 text-right shrink-0">{labelRight}</div>
    </div>
  )
}
