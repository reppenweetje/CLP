...
| 2026-05-08  | **Brevo retry-fix plus dual-write** (commits `b89fa61` en `11f4d86`). Eerste echte lead landde wel in Supabase maar niet in Brevo door 400 `duplicate_parameter` op SMS. Brevo enforced uniqueness op het SMS-attribuut. Twee fixes in `lead-upsert/brevo.ts`: (1) detecteer 400 op SMS-conflict en retry zonder SMS zodat email + andere attributes wel landen, (2) telefoonnummer wordt nu naar zowel SMS als nieuw custom attribuut WHATSAPP geschreven zodat sales altijd het 06 kan vinden, ook als SMS leeg blijft door conflict. Edge Function 2x geredeployed door Jann via tijdelijke access token (token meteen weer revoked). Code in repo matcht nu de live deployment. |
